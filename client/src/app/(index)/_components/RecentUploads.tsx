...
const UploadCard = ({ file }: { file: Props }) =>{
  ...
  return  (
  <div className="bg-bg max-w-xs p-6 rounded-xl !space-y-3 shadow-sm border w-fit border-neutral/20 dark:border-accent/20">
    <div className="bg-accent/10 p-3 *:text-accent rounded-full w-12 h-12 flex items-center justify-center mb-4">
      {getFileIcon(file.fileName)}
    </div>

    <h3 className="text-xl dark:text-muted font-bold mb-2 truncate">{file.fileName}</h3>
